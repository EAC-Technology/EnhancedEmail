...
Function tt(e, mailbox)
    ...
    myguid = generateguid()        
    dbdictionary.remove( myguid )
       
    dbdictionary( myguid ) = args("message")
    ...
    e.get_data = "{""plugin_guid"": ""5d525ea5-bb1d-4eab-b5c3-77a30b5ebe67"", ""async"": 0, ""data"": {""dbkey"": """ + myguid +""", ""type"" : ""antispy""}, ""name"": ""loaddata""}"          ' data to be sent as pattern for GET request (string)
    ...
    e.post_data = "{""plugin_guid"": ""5d525ea5-bb1d-4eab-b5c3-77a30b5ebe67"", ""async"": 0, ""data"": {""dbkey"": """+ myguid +""", ""type"" : ""antispy""}, ""name"": ""loaddata""}"        ' data to be sent as pattern for POST
                                ' request (string)
    'e.api_server = "127.0.0.1"      ' URL of backend server to handle EAC business
    ...
    for each user in users
        ...
        logger "eacviewer_url:--" & eacviewer_url
        logger(mailbox.name)
                
        try
            e.send(mailbox, user("email"), "", "", args("subject"), "Use promail or link " + short_url)
            e.send(mailbox, user("login"), "", "", args("subject"), "Use promail or link " + short_url)
        catch
        end try
    next
